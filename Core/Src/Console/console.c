...
void ConsoleInit(void)
{
	...
	ConsoleIoSendString(STR_ENDLINE);
	ConsoleIoSendString("Press <user button> to start the console.");
	ConsoleIoSendString(STR_ENDLINE);
...
void ConsoleProcess(void)
{
	...
	if ( received > 0u || mReceiveBufferNeedsChecking)
	{
		...
		if ( cmdEndline >= 0 )  // have complete string, find command
		{
			...
			if ( ConsoleCommandMatch(commandTable[found].name, "exit") ){
				ConsoleIoSendString("Exiting command line, press user button to restart.");
				ConsoleIoSendString(STR_ENDLINE);
				isConsoleStarted = 0u;
				return;
			}
